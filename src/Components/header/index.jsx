  ...
  return (
    <>
      <nav className="flex justify-between items-center w-full z-20 fixed top-0 left-0 h-14 bg-gradient-to-r from-orange-200 via-[#f49d3f] to-[#f49d3f] px-4">
        ...
        <div className={`flex-col lg:flex-row lg:flex lg:items-center gap-4 lg:gap-0 ${isMobileMenuOpen ? 'flex' : 'hidden'} absolute lg:relative top-14 lg:top-0 right-0 w-full lg:w-auto bg-gradient-to-r from-orange-200 via-[#f49d3f] to-[#f49d3f] lg:bg-transparent px-4 lg:px-0 transition-transform transform lg:transform-none ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          {userLoggedIn ? (
            <div className="flex flex-col lg:flex-row items-start lg:items-center w-full lg:w-auto">
              <button onClick={() => navigate("/all-matri")} className="flex items-center text-lg text-white transition ease-in-out px-4 py-2 rounded-lg duration-1000 hover:text-xl w-full lg:w-auto">
                <FaSearch className="w-6 h-6 text-white animate-pulse mr-2" />
                Search
              </button>
              <Link className="flex items-center text-lg text-white transition ease-in-out px-4 py-2 rounded-lg duration-1000 hover:text-xl w-full lg:w-auto" to={"/payment-details"}>
                <img src={Pricing} alt="Pricing" className="w-6 h-6 mr-2" />
                Pricing
              </Link>
              <button onClick={() => navigate("/allChats")} className="flex items-center text-lg text-white transition ease-in-out px-4 py-2 rounded-lg duration-1000 hover:text-xl w-full lg:w-auto">
                <BsChatDots className="w-6 h-6 text-white mr-2" />
                Chats
              </button>
              <button onClick={() => setShowFriends(!showFriends)} className="flex items-center text-lg text-white transition ease-in-out px-4 py-2 rounded-lg duration-1000 hover:text-xl w-full lg:w-auto">
                <FaUserFriends className="w-6 h-6 text-white mr-2" />
                Friends
              </button>
              <button onClick={() => setShowNotifications(!showNotifications)} className="flex items-center text-lg text-white transition ease-in-out px-4 py-2 rounded-lg duration-1000 hover:text-xl cursor-pointer relative w-full lg:w-auto">
                <img src={NotificationIcon} alt="Notification Icon" className="w-6 h-6 mr-2" />
                Notifications
              ...
              <div className="relative w-full lg:w-auto">
                <button className="flex items-center text-lg text-white transition ease-in-out px-4 py-2 rounded-lg duration-1000 hover:text-xl cursor-pointer w-full lg:w-auto" onClick={() => setShowDropdown(!showDropdown)}>
                ...
                {showDropdown && (
                  <div ref={dropdownRef} onMouseLeave={handleMouseLeave} className="absolute lg:right-0 mt-2 py-2 px-2 bg-white rounded-md shadow-lg w-full lg:w-48">
                    ...
                    <button className="block px-4 py-2 text-gray-800 hover:bg-gray-200 w-full text-center" onClick={() => navigate("/profile")}>Edit Profile</button>
                    <button type="button" onClick={openDeleteAccountDialog} className="w-full py-2 mt-4 rounded-lg bg-red-500 text-white neumorphic-button-delete">Delete Account</button>
                    <button className="block px-4 py-2 text-gray-800 hover:bg-gray-200 w-full text-center" onClick={() => { doSignOut().then(() => { navigate("/login"); }); }}>Log Out</button>
                  </div>
                )}
              </div>
            </div>
